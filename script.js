(function () {
  const defaultLang = (navigator.language || 'en').toLowerCase().startsWith('de') ? 'de' : 'en';
  const storedLang = localStorage.getItem('lang') || defaultLang;

  // Only replace text if the key exists in the chosen language
  function setLang(lang) {
    ...
    document.querySelectorAll('[data-i18n]').forEach((el) => {
      ...
      // If key is missing, leave the existing HTML/text alone
    });
    localStorage.setItem('lang', lang);
  }

  window.setLanguage = setLang; // used by ENG | DE buttons
  ...
  const translations = {
    en: {
      ...
      about_me_text:
        "I’m a Media Design graduate with a passion for visual storytelling, video editing, and outdoor adventure. Whether behind the camera, in the edit bay, or crafting concepts, I thrive on bringing ideas to life. My work spans branded content, documentary storytelling, and creative social media formats.",
  ...
  // Apply after DOM is ready (safer on Cargo pages)
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setLang(storedLang));
  } else {
    setLang(storedLang);
  }
})();
